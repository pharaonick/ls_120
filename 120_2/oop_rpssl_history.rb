...
# it all works but just feels... awkward. Like, have I made things easier than procedural?
# should I have linked classes together better/more/differently?
# (all a bit contrived since were given classes to work with...)
# feels like i'm using objects procedurally!

# explain how took different tack for solving initial rubcop complaint about winning moves
# (having custom and string objects here as possible values of `winner` is awkward?)

# have tried to be consistent re: use of ivar vs getter/setter (started out using ivar where pos to reduce
# method calls but it's ugly so went with getter/setter. Advice?)

# go through attr_accessors, make sure not defining ones not needed (will prolly need a lot of reader)

  # computer.choose: Can't figure out how to reduce cyclomatic complexity further.
  # changing self.move to @move hasn't helped
  # Maybe having personalities as classes.

  # left some method length (in one case it's too long because of output vars)
  # left a coule line length where felt correcting would make it worse

# explain very simple quicklearner logic...
